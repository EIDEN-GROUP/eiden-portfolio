  ...
  return (
    ...
    >
      <img
        src={q.cover}
        alt={q.title}
        className="absolute inset-0 h-full w-full object-cover transition-transform duration-[2s] ease-[cubic-bezier(.22,1,.36,1)] group-hover:scale-[1.04]"
      />
      <div
        className="absolute inset-0"
        style={{
          background:
            "linear-gradient(to top, color-mix(in srgb, var(--project-bg) 92%, transparent), color-mix(in srgb, var(--project-bg) 45%, transparent) 50%, transparent)",
        }}
      />

      <span
        className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 select-none font-display font-bold leading-none tracking-[-0.06em] opacity-[0.06]"
        style={{ fontSize: "clamp(6rem, 16vw, 14rem)", color: "var(--project-text)" }}
        aria-hidden
      >
        {q.index}
      </span>
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-4 px-5 pb-7 sm:flex-row sm:items-end sm:justify-between sm:gap-4 sm:px-7 sm:pb-10">
        <div className="min-w-0 flex-1">
          <p
            className="mb-2 font-label text-[9px] uppercase tracking-[0.46em]"
            style={{ color: "var(--project-accent)" }}
          >
            {q.index} · {q.category}
          </p>
          <h3
            className="font-display font-bold leading-[1.02] tracking-[-0.04em] transition-opacity group-hover:opacity-90"
            style={{
          ...
          <p
            className="mt-2 max-w-md font-editorial text-sm italic leading-relaxed sm:text-base"
  ...
  return (
    ...
    >
      <div>
        ...
        <Link
          to="/"
          className="font-label text-[11px] uppercase tracking-[0.3em] text-gold hover:underline"
        >
          ← Back to portfolio
        </Link>
      </div>
...
function ProjectDetailHero({ project: p }: { project: Project }) {
  ...
  const heroY = useTransform(scrollYProgress, [0, 1], theme.motion.heroParallax);
  ...
  return (
    <section
      ref={heroRef}
      className="relative min-h-[100svh] w-full overflow-hidden supports-[height:100dvh]:min-h-[100dvh]"
    >
      <motion.img
        src={p.detailHero}
        alt={p.title}
      ...
      <div className="absolute inset-0" style={{ background: theme.gradients.heroScrim }} />
      <div className="absolute inset-0" style={{ background: theme.gradients.hero }} />

      {theme.ui.heroFilmGrain ? (
        <motion.div
          aria-hidden
          className="pointer-events-none absolute inset-0 mix-blend-overlay"
          style={{
            opacity: theme.ui.grainOpacity,
      ...
      <span
        className="pointer-events-none absolute right-0 top-1/2 max-sm:-right-4 max-sm:top-[58%] -translate-y-1/2 select-none font-display font-bold leading-none tracking-[-0.06em] opacity-[0.07]"
        style={{
      ...
      <div className="absolute left-[max(1rem,env(safe-area-inset-left))] right-[max(1rem,env(safe-area-inset-right))] top-[calc(4.5rem+env(safe-area-inset-top))] flex items-center justify-between sm:left-12 sm:right-12 sm:top-[calc(6rem+env(safe-area-inset-top))]">
        <Link
          to="/"
          className="flex items-center gap-2 font-label text-[9px] uppercase tracking-[0.48em] transition-colors hover:opacity-100"
      ...
      <motion.div
        ...
        className="absolute inset-x-0 bottom-0 px-[max(1rem,env(safe-area-inset-left))] pb-[max(3rem,env(safe-area-inset-bottom))] pt-6 sm:px-12 sm:pb-16 sm:pt-0"
      >
        <motion.p
        ...
        <motion.h1
          ...
          variants={{
            visible: {
              transition: { staggerChildren: theme.motion.stagger, delayChildren: 0.12 },
            },
          }}
          className="font-display font-bold leading-[1.03] tracking-[-0.04em] max-sm:break-words sm:leading-[1.01]"
          style={{
            fontSize: "clamp(1.85rem, min(5.5vw, 8vh), 5rem)",
            color: theme.colors.text,
          }}
        >
        ...
        <motion.p
          ...
          className="mt-4 max-w-2xl font-editorial text-base leading-relaxed sm:mt-5 sm:text-lg md:text-xl"
          style={{ color: theme.colors.textMuted }}
        >
          {p.tagline}
        </motion.p>
        ...
        <motion.p
          ...
          className="mt-3 max-w-full font-label text-[9px] uppercase tracking-[0.36em] sm:mt-4 sm:text-[10px] sm:tracking-[0.42em] md:text-[11px]"
          style={{ color: theme.colors.textMuted }}
        >
          {deliverablesLine}
        </motion.p>
      ...
      >
        <div className="mx-auto max-w-7xl px-[max(1rem,env(safe-area-inset-left))] py-12 pr-[max(1rem,env(safe-area-inset-right))] sm:px-12 sm:py-20">
          <div className="mb-8 flex flex-col gap-3 sm:mb-12 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              to="/"
              className="self-start font-label text-[8px] uppercase tracking-[0.44em] transition-opacity hover:opacity-100 sm:self-auto"
              style={{ color: theme.colors.textMuted }}
            >
              All projects →
            </Link>
